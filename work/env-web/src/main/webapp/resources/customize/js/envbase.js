var Envbase = function($) {
	...
	var handleTooltip = function() {
		var ctx = arguments.length > 0 ? arguments[0] : null;
		if (Envbase.isTouchDevice()) { // if touch device, some tooltips can be
	...
	var validation_option = {
		...
		errorPlacement : function(errors, element) {
			...
			if (isChosen) {
				...
				var $container = element.siblings(".chzn-container");
				if (isMultiple) {
					target = $container.find("ul");
				} else {
					//target = $container.find("a.chosen-single");
					target = $container.find("a.chzn-single");
				}
			}
            ...
            target[0].setAttribute('data-original-title',error.innerHTML);//火狐没有innerText方法，jQuery的attr方法存在bug。
					...
					invalidHandler : function(event, validator) {
						...
						if (errors) {
							...
							if (!ctx) {
								 ...
								 * $("#error_message").show();
								 */
								EnvbaseUtil.showAlertMessage("#error_message",
								 ...
								 */
								/*
								 * EnvbaseUtil.showAlertMessage(ctx,
								 * "表单中有错误，请完善表单后再提交。", 'error');
								 */
								EnvbaseUtil.showAlertMessage($("#error_message",
	...
	var handleLazySection = function() {
		$(".lazy-section").each(function() {
			...
			EnvbaseUtil.ajaxTurn2Page($(this), baseUrl, 1);
		});
	};
...
var EnvbaseUtil = function($) {
	...
	var handleAjaxTurn2Page = function() {
		...
		$(contextSel).load(baseUrl, param, function() {
			// Envbase.unblockUI(contextSel);
			Envbase.initContext(contextSel);
			$(contextSel).trigger({
				type : "pageLoaded",
				number : number
			});
	...
	var handleCreateModal = function() {
		...
		handleBaseCreateModal(title, function() {
			...
			Envbase.initContext("#modal_body");
